...
/datum/element/electrovoreable/proc/on_attack_self(obj/item/source, mob/user)
	...
	if(!istype(source, /obj/item/stock_parts/cell))
		return
	var/obj/item/stock_parts/cell/source_cell = source

	if(living_user.a_intent == INTENT_HELP && HAS_TRAIT(living_user, TRAIT_ELECTROVORE_OBLIGATE))
	...
	if(living_user.a_intent == INTENT_HURT)
