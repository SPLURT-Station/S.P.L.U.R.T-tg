...
/mob/living/basic/slime/proc/add_crown(obj/item/attacking_item, mob/living/user)
	...
	if(istype(attacking_item, /obj/item/clothing/head/costume/crown) && stat == CONSCIOUS && !wearing_crown)
		wearing_crown = TRUE
		// remove the used item and update overlays (our update_overlays override will re-apply the crown overlay)
		del attacking_item
		update_overlays()
...
/mob/living/basic/slime/proc/update_crown_overlay()
	// Adds the appropriate crown overlay depending on life stage. Intended to be
	// called after base overlay updates so it isn't cleared.
	if(wearing_crown)
...
// Ensure the crown overlay is applied after the base overlay update pass so it
// isn't cleared by cut_overlays()/regenerate icons. This mirrors the modular
// implementation that overrides update_overlays.
/mob/living/basic/slime/proc/update_overlays()
	// Call parent overlay updater then append crown overlay if needed.
	..()
