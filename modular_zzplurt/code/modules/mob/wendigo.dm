...
/mob/living/basic/wendigo
	...
	icon = 'icons/mob/lavaland/megafauna.dmi'
	icon_state = "wendigo" // Placeholder icon, we will need to add a proper one if SPLURT has it.
	icon_living = "wendigo"
...
/mob/living/basic/wendigo/death()
	playsound(src, 'sound/effects/megafauna/wendigo_death.ogg', 100, TRUE, falloff_distance = 10) // Placeholder
	..()
	gib()
...
/mob/living/basic/wendigo/proc/do_scream()
	...
	playsound(src, 'sound/effects/scream.ogg', 100, TRUE)
	for(var/mob/living/M in oview(7, src))
		if(M.stat != DEAD && !M.has_trait(TRAIT_DEAF))
			M.apply_effect(100, CONFUSION)
