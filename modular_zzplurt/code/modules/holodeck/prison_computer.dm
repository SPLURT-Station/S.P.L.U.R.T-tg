...
/obj/machinery/computer/holodeck_prison
	...
	var/area/mapped_start_area = /area/station/holodeck/prison
	...
	var/program_type = /datum/map_template/holodeck_prison
...
/obj/machinery/computer/holodeck_prison/post_machine_initialize() //from here linked is populated and the program list is generated. its also set to load the offline program
	. = ..()
...
/obj/machinery/computer/holodeck_prison/ui_interact(mob/user, datum/tgui/ui)
	. = ..()
...
/obj/machinery/computer/holodeck_prison/ui_data(mob/user)
	var/list/data = list()
...
/obj/machinery/computer/holodeck_prison/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	. = ..()
	if(.)
		return
	. = TRUE
...
/datum/map_template/holodeck_prison/update_blacklist(turf/placement, list/input_blacklist)
...
/obj/machinery/computer/holodeck_prison/proc/load_program(map_id, force = FALSE, add_delay = TRUE)
	if (program == map_id)
		return
...
/obj/machinery/computer/holodeck_prison/proc/reset_to_default()
...
/obj/machinery/computer/holodeck_prison/proc/clear_projection()
...
/obj/machinery/computer/holodeck_prison/proc/finish_spawn()
...
/obj/machinery/computer/holodeck_prison/proc/finalize_spawned(atom/holo_atom)
...
/obj/machinery/computer/holodeck_prison/proc/register_contents(obj/structure/closet/storage)
	SIGNAL_HANDLER
 ...
 */
/obj/machinery/computer/holodeck_prison/proc/add_to_spawned(atom/holo_atom)
	spawned |= holo_atom
...
/obj/machinery/computer/holodeck_prison/proc/derez(atom/movable/holo_atom, silent = TRUE, forced = FALSE)
	spawned -= holo_atom
	if(!holo_atom)
		return
...
/obj/machinery/computer/holodeck_prison/proc/remove_from_holo_lists(datum/to_remove, _forced)
	SIGNAL_HANDLER
	spawned -= to_remove
	UnregisterSignal(to_remove, COMSIG_QDELETING)

/obj/machinery/computer/holodeck_prison/process(seconds_per_tick)
...
/obj/machinery/computer/holodeck_prison/proc/toggle_power(toggleOn = FALSE)
	if(active == toggleOn)
		return
...
/obj/machinery/computer/holodeck_prison/proc/nerf(nerf_this, is_loading = TRUE)
...
/obj/machinery/computer/holodeck_prison/emag_act(mob/user, obj/item/card/emag/emag_card)
	if(obj_flags & EMAGGED)
		return FALSE
...
/obj/machinery/computer/holodeck_prison/emp_act(severity)
	. = ..()
	if(. & EMP_PROTECT_SELF)
		return
	emergency_shutdown()

/obj/machinery/computer/holodeck_prison/ex_act(severity, target)
	emergency_shutdown()
	return ..()

/obj/machinery/computer/holodeck_prison/Destroy()
	reset_to_default()
	if(linked)
		linked.linked = null
		linked.energy_usage = list(AREA_USAGE_LEN)
...
/obj/machinery/computer/holodeck_prison/blob_act(obj/structure/blob/B)
	emergency_shutdown()
	return ..()
