...
/obj/item/organ/brain/slime/attack_self(mob/living/user) // Allows a player (presumably an antag) to deactivate the GPS signal on a slime core
	...
	user.visible_message(
		span_warning("[user] crunches something deep in the slime core! It gradually stops glowing..."),
...
/obj/item/organ/brain/slime/proc/on_slime_death(mob/living/victim, gibbed)
	...
	if(gibbed) // organ spillage is handled for us
		return
...
/obj/item/organ/brain/slime/check_for_repair(obj/item/item, mob/user)
	...
	if(item.reagents.get_reagent_amount(/datum/reagent/toxin/plasma) < 100)
		user.balloon_alert(user, "not enough plasma to heal them fully!")
		return TRUE // its a fucking attackby hook
...
/datum/species/jelly/roundstartslime/create_pref_unique_perks()
	...
	to_add += list(
		...
		list(
			...
			SPECIES_PERK_DESC = "Slimes are capable of cleaning themselves and their clothing, siphoning the dirt off it and into themselves; even off the floor, if they're barefoot.",
		),
		list(
			SPECIES_PERK_TYPE = SPECIES_POSITIVE_PERK,
...
/datum/action/innate/core_signal
	...
	button_icon = 'modular_zzplurt/icons/obj/medical/organs/organs.dmi'
